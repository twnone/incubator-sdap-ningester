...
public class CassandraStore implements DataStore {
    ...
    public CassandraStore(CassandraTemplate cassandraTemplate) {
    ...
    public void saveData(List<? extends NexusTile> nexusTiles) {
        ...
        nexusTiles.stream()
		.map(nexusTile -> cassandraTemplate.insert(getCassandraPojo(nexusTile.getTile())))
		.collect(Collectors.toList());
    }

    private CassandraReadyNexusTile getCassandraPojo(TileData tile) {
        CassandraReadyNexusTile pojo = new CassandraReadyNexusTile(UUID.fromString(tile.getTileId()), ByteBuffer.wrap(tile.toByteArray()));
        return pojo;
    }
